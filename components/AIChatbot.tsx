...
import { Bot, X, Send, User, Sparkles, Minus, Maximize2, MessageCircle } from 'lucide-react';
...
const AIChatbot: React.FC = () => {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      // Fixed: Always use direct process.env.API_KEY when initializing GoogleGenAI
      ...
      // Fixed: Extracting text directly from property 'text'
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                  <div className={`flex gap-2 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 shadow-lg ${msg.role === 'user' ? 'bg-red-600' : 'bg-white/10'}`}>
                      {msg.role === 'user' ? <User size={14} className="text-white" /> : <Bot size={14} className="text-red-500" />}
                    </div>
