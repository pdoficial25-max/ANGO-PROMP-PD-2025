      ...
      <div className="bg-[#141414] border border-white/5 rounded-[24px] md:rounded-[32px] p-4 md:p-6 mb-4 group transition-all duration-300 hover:border-red-600/20 relative overflow-hidden">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <img src={post.userAvatar} className="w-10 h-10 md:w-11 md:h-11 rounded-full border border-white/10 object-cover" alt={post.userName} />
            <div>
        ...
        <div className="mb-4">
          ...
          {!isEditingPost && renderMedia(false)}
        </div>
              ...
              {(post.comments || []).map((comment) => {
                ...
                return (
                  <div key={comment.id} className="flex gap-3 group/comment">
                    <img src={comment.userAvatar} className="w-8 h-8 md:w-9 md:h-9 rounded-full shrink-0 border border-white/5 object-cover" alt="" />
                    <div className="flex-1 space-y-1">
                      <div className="bg-white/5 rounded-2xl p-3 md:p-4 border border-white/5 relative">
                        ...
                        {!isEditingThisComment && commentReactionsCount > 0 && (
                      ...
                      {!isEditingThisComment && (
                        <div className="flex items-center gap-4 px-2">
                          <div 
                            className="relative"
                            onMouseEnter={() => {
                              if (window.innerWidth >= 1024) setActiveCommentMenuId(comment.id);
                            }}
                            onMouseLeave={() => setActiveCommentMenuId(null)}
                          >
                            <button 
                              onClick={() => {
                          ...
                          {comment.userId === currentUserId && (
                            <div className="flex gap-3 opacity-0 group-hover/comment:opacity-100 transition-opacity">
                              ...
                              <button onClick={() => { if(confirm('Eliminar este comentário?')) onDeleteComment?.(comment.id); }} className="text-[9px] font-black text-gray-700 hover:text-red-500 uppercase tracking-widest">Eliminar</button>
                            </div>
                          )}
                        </div>
                      )}
          ...
          <div className="relative w-full max-w-4xl bg-[#0f0f0f] border border-white/10 rounded-[32px] md:rounded-[40px] overflow-hidden flex flex-col max-h-[90vh] shadow-2xl animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-white/5">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-black text-white text-sm md:text-lg uppercase tracking-tight text-center">{post.userName}</h3>
              </div>
